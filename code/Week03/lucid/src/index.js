...
const vestingScript = {//we need the validator script here
    type: "PlutusV2",
...
const VestingDatum = L.Data.Object({ // the datum converted into json data 
    beneficiary: L.Data.String,
    deadline: L.Data.BigInt,
});

function removeChildren(elt) { //UI
    while (elt.firstChild) {
        elt.removeChild(elt.lastChild);
    }
}

async function loadCardano() {// initialization of lucid and connection with nami wallet
...
async function submitCardanoTx(signedTx) {//helper function to submit a transaction
